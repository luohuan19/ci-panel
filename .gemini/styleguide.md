...
- **All four packages have vitest suites.** The gate is those suites plus
  `type-check` (all four), `lint` (frontend only) and `build` (all four). Report
  real counts rather than claiming "tests pass".
