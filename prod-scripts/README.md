...
| 启停 runner | `sudo -n systemctl start\|stop\|restart` | 面板上点启停 |
...
2. 由两个 `.sudoers` 模板生成实际规则（替换用户名与命令绝对路径），`visudo` 校验后落盘
...
| `ci-panel-runner.sudoers` | 模板：启停三条白名单 → `/etc/sudoers.d/ci-panel-runner` |
| `ci-panel-runner-install.sudoers` | 模板：助手一条白名单 → `/etc/sudoers.d/ci-panel-runner-install` |
| `linux/`、`windows/` | 上游 MCSManager 的启动脚本，与 runner 无关 |

## 权限边界

授权面只有上表那两条，root 实际能做的事全部写在 `ci-panel-runner-svc` 里，并受这些约束：
...
```
[runner-scan] 扫描根取自特权助手(v1): /data/ci-runner
```
